...
- `codegen.generate(pnml)` -> { files: {path: content}, entrypoint: path, requirements: [] }
  - Current implementation returns an in-memory artifact with a single entrypoint file.
  - It does not persist generated files to disk by default.
- `runtime.run_in_venv(code_artifact, options)` -> { run_id, stdout, stderr, exit_code, start_time, end_time }
  - Current implementation writes the artifact to a temporary directory (e.g., /tmp/evolve_run_*) and executes from there.
  - Temporary files are cleaned up after the run completes.
- For an editable on-disk project layout, use `project_gen.generate_python_project(yaml_text, out_dir, source_name)`.
  - This generates a package directory with `main.py`, `inscriptions.py`, and a vendored `enginepy/` runtime.
- Token payloads should include the entrypoint path and optional run metadata for execution (for example, `entry` and a shell command for `t_run`).
- Run metadata shape (current):
  - `code.run.command`: array command (e.g., `["python", "main.py"]`).
  - `code.run.cwd`: working directory (default `"."`).
  - `code.run.entrypoint`: entrypoint path for convenience.
- Preservation / Debugging helpers:
  - Set environment variable `EVOLVE_PRESERVE_RUNS=1` to preserve run directories by default (the runner will return `tmp_dir` and `preserved` in the result and print `preserved run directory: <path>` to stderr).
...
- Unit tests:
  ...
  - `test_project_gen_writes_project` (ensures on-disk project generation produces entrypoint and inscriptions).
